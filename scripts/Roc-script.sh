...
# ==================== 1. 初始化feed + 补齐缺失依赖【关键修复2errors】 ====================
green "===== 1/15 Update & Install Feeds ====="
./scripts/feeds update -a
./scripts/feeds install -a
# 补齐lxc、onionshare缺失依赖
...
[ -f /etc/init.d/network ] && /etc/init.d/network restart 2>/dev/null
...
for cpu in /sys/devices/system/cpu/cpu[1-9]*/online; do
    [ -w "$cpu" ] && echo 1 > "$cpu" 2>/dev/null
done
if [ -d /sys/devices/system/cpu/cpufreq ]; then
    for policy in /sys/devices/system/cpu/cpufreq/policy*; do
        [ -w "$policy/scaling_governor" ] && echo "ondemand" > "$policy/scaling_governor" 2>/dev/null
        [ -w "$policy/scaling_min_freq" ] && echo 800000  > "$policy/scaling_min_freq" 2>/dev/null
        [ -w "$policy/scaling_max_freq" ] && echo 1800000 > "$policy/scaling_max_freq" 2>/dev/null
    done
    [ -w /sys/devices/system/cpu/cpufreq/ondemand/up_threshold ] && echo 15     > /sys/devices/system/cpu/cpufreq/ondemand/up_threshold 2>/dev/null
    [ -w /sys/devices/system/cpu/cpufreq/ondemand/sampling_rate ] && echo 5000   > /sys/devices/system/cpu/cpufreq/ondemand/sampling_rate 2>/dev/null
    [ -w /sys/devices/system/cpu/cpufreq/ondemand/ignore_nice_load ] && echo 0      > /sys/devices/system/cpu/cpufreq/ondemand/ignore_nice_load 2>/dev/null
    [ -w /sys/devices/system/cpu/cpufreq/ondemand/sampling_down_factor ] && echo 1000   > /sys/devices/system/cpu/cpufreq/ondemand/sampling_down_factor 2>/dev/null
fi
exit 0
EOF
